...
Generated 2026-09-25T11:56:43.359Z by `scripts/net/` (build f4cc5804285b). The page reads these files same-origin after the
...
| `apple-relay` | [Apple — iCloud Private Relay egress IP ranges](https://mask-api.icloud.com/egress-ip-ranges.csv) | relay, determined | 52259 | 2026-09-25T11:56:27.213Z | fresh | Published by Apple so that sites can recognise Private Relay traffic; no licence text |
...
| `ivpn` | [IVPN — server list](https://api.ivpn.net/v5/servers.json) | vpn, determined | 178 | 2026-09-25T11:56:31.017Z | fresh | Operator's public server list (app API; no published terms); only the derived address → operator map is shipped. Entry (server) addresses. Exits usually match for WireGuard servers but not always; a miss means nothing. |
| `nordvpn` | [NordVPN — server list (undocumented public API)](https://api.nordvpn.com/v1/servers) | vpn, determined | 7674 | 2026-09-25T11:56:31.000Z | fresh | Operator's public server list (app API; no published terms); only the derived address → operator map is shipped. Entry (server) addresses. Exits usually match for WireGuard servers but not always; a miss means nothing. |
| `pia` | [Private Internet Access — server list](https://serverlist.piaservers.net/vpninfo/servers/v6) | vpn, determined | 1595 | 2026-09-25T11:56:31.000Z | fresh | Operator's public server list (app API; no published terms); only the derived address → operator map is shipped. Entry (server) addresses. Exits usually match for WireGuard servers but not always; a miss means nothing. |
| `zscaler` | [Zscaler — cloud egress ranges (cenr, all Zscaler clouds)](https://config.zscaler.com/zscaler.net/cenr) | corporate-proxy, determined | 941 | 2026-09-25T11:56:34.442Z | fresh | Published by Zscaler for customer firewall configuration; no licence text |
...
| `vultr` | [Vultr (Constant) — geofeed](https://geofeed.constant.com/) | datacenter, determined | 466 | 2026-09-25T11:56:36.546Z | fresh | Self-published RFC 8805 geofeed; no licence text |
...
| `cloudflare` | [Cloudflare — CDN / reverse-proxy edge ranges (ips-v4, ips-v6)](https://www.cloudflare.com/ips/) | cdn, determined | 22 | 2026-09-25T08:55:21.000Z | fresh | Published by Cloudflare; no licence text |
| `fastly` | [Fastly — public IP list (CDN edge)](https://api.fastly.com/public-ip-list) | cdn, determined | 21 | 2026-09-25T11:56:37.049Z | fresh | Published by Fastly; no licence text |
